...
module.exports.apply = async (app, options, storage) => {
  ...
  app.middleware(async (meta, next) => {
    if (!meta.content.startsWith('!!stat'))
      return next()
    let mode
    const command = meta.content.split(' ')
    const username = unescapeSpecialChars(command.slice(1).join(' ').trim())
    if (!username)
      return meta.send('提供一下用户名。 !!stat(@模式:[osu, taiko, fruits, mania]) osuid\nex: !!stat arily, !!stat@mania arily')

    if (!command[0].includes('@'))
      mode = undefined
    mode = command[0].split('@')[1]
    if (!['osu', 'taiko', 'fruits', 'mania', undefined].includes(mode))
      return meta.send(`模式有 osu, taiko, fruits, mania. ${mode}不在其中。`)
  ...
  app.middleware(async (meta, next) => {
    if (!meta.content.startsWith('!!pr') && !meta.content.startsWith('!!recent'))
      return next()
    let mode
    const command = meta.content.split(' ')
    const username = unescapeSpecialChars(command.slice(1).join(' ').trim())
    if (!username)
      return meta.send('提供一下用户名。 !!pr(@模式:[osu, taiko, fruits, mania]) osuid\nex: !!pr arily, !!pr@mania arily')
  ...
  app.middleware(async (meta, next) => {
    if (!meta.content.startsWith('!!userpage'))
      return next()
    const command = meta.content.split(' ')
    const username = unescapeSpecialChars(command.slice(1).join(' ').trim())
    if (!username)
      return meta.send('提供一下用户名。 !!userpage osuid\nex: !!userpage arily')
  ...
  app.middleware(async (meta, next) => {
    if (!meta.content.startsWith('!!best'))
      return next()
    let mode
    const command = meta.content.split(' ')
    ...
    const params = command.filter(c => c.startsWith('@')).reduce((acc, command) => {
      if (command.startsWith('@last:'))
        acc.startHoursBefore = command.slice(6)
      if (command.startsWith('@from:'))
        acc.start = command.slice(6)
      if (command.startsWith('@to:'))
        acc.end = command.slice(4)
      return acc
    }, {})
    if (Object.keys(params).length === 0)
      params.startHoursBefore = 24
    const reply = []
