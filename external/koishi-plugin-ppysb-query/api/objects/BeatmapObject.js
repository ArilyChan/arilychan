...
class BeatmapObject {
  constructor(beatmap, isRipple) {
    ...
    if (!isRipple)
      this.mode = Number.parseInt(beatmap.mode)
    if (!isRipple)
      this.beatmapMode = Number.parseInt(beatmap.mode)
    if (!isRipple)
    ...
    if (!isRipple)
      this.stars = Number.parseFloat(beatmap.difficultyrating)
    if (isRipple)
  ...
  toString() {
    ...
    if (this.isRippleApi) {
      ...
      if (this.difficulty2.std > 0)
        output = `${output}std： ${this.difficulty2.std}★  `
      if (this.difficulty2.taiko > 0)
        output = `${output}taiko： ${this.difficulty2.taiko}★  `
      if (this.difficulty2.ctb > 0)
        output = `${output}ctb： ${this.difficulty2.ctb}★  `
      if (this.difficulty2.mania > 0)
        output = `${output}mania： ${this.difficulty2.mania}★  `
      output = `${output}\n`
    }
    else {
  ...
  toScoreTitle(scoremode) {
    if (this.isRippleApi) { // scoremode应为score.play_mode
      let diff = ''
      if (scoremode === 0)
        diff = ` ★${this.difficulty2.std.toFixed(2)}`
      else if (scoremode === 1)
        diff = ` ★${this.difficulty2.taiko.toFixed(2)}`
      else if (scoremode === 2)
        diff = ` ★${this.difficulty2.ctb.toFixed(2)}`
      else if (scoremode === 3)
        diff = ` ★${this.difficulty2.mania.toFixed(2)}`
