...
class getBestScoresData {
  ...
  async getBestScoresObject(simpleUserObject) {
    ...
    if (result.code === 404)
      throw `找不到成绩 ${utils.apiObjectToString(this.apiObject)}`
    if (result.code === 400)
      throw '必须指定玩家名或Id（或先setid绑定私服账户）'
    if (result.code === 'error')
      throw `获取成绩出错 ${utils.apiObjectToString(this.apiObject)}`
    const scores = result.scores
    if ((!Array.isArray(scores)) || (scores.length <= 0))
      throw `找不到成绩 ${utils.apiObjectToString(this.apiObject)}`
    const scoreObjects = scores.map((item) => { return new ScoreObject(item, null, simpleUserObject) })
    return scoreObjects
  }
  ...
  async getSimpleUserObject() {
    ...
    const userArgObject = {}
    if (this.apiObject.u)
      userArgObject.u = this.apiObject.u
    if (this.apiObject.type)
      userArgObject.type = this.apiObject.type
  ...
  async outputToday() {
    try {
      ...
      const simpleUserObject = await this.getSimpleUserObject()
      // bp列表
      let output = ''
      const scoreObjects = await this.getBestScoresObject(simpleUserObject)
      let todayScoreObjects = scoreObjects.filter((scoreObject) => {
        return (today - scoreObject.getPlayedDate().getTime() < 24 * 3600 * 1000)
      })
      if (todayScoreObjects.length <= 0)
        return '您今天还没刷新bp呢（你气不气.wav）'
      let over15Count = 0
      ...
      if (over15Count > 0)
        output = `${output}为防止文字过长，已省略剩余的 ${over15Count} 个bp`
      return output
    }
    catch (ex) {
      return ex
  ...
  async output() {
    try {
      ...
      if (this.apiObject.limit) {
        ...
        const scoreObjects = await this.getBestScoresObject(simpleUserObject)
        const scoreObject = scoreObjects[0]
        let output = ''
        output = output + scoreObject.beatmap.toScoreTitle(scoreObject.mode)
  ...
  async outputBpNumber() {
    try {
      ...
      // bp列表
      let output = ''
      const scoreObjects = await this.getAllBestScoresObject(simpleUserObject)
      const length = scoreObjects.length
      const beatmapId = this.apiObject.b
      ...
      if (this.searchText) {
        for (let i = 0; i < length; i++) {
          if (scoreObjects[i].beatmap.songName.toLowerCase().includes(this.searchText.toLowerCase())) {
            output = output + scoreObjects[i].beatmap.toScoreTitle(scoreObjects[i].mode)
            output = output + await scoreObjects[i].toCompleteString(this.exscore)
            output = `${output}\n该谱面是您的bp ${(i + 1).toString()}`
            return output
  ...
  async outputRankNumber() {
    try {
      ...
      const scoreObjects = await this.getAllBestScoresObject(simpleUserObject)
      const rank_list = ['SSH', 'SS', 'SH', 'S', 'A', 'B', 'C', 'D']
      const rank_count = [0, 0, 0, 0, 0, 0, 0, 0]
      const length = scoreObjects.length
      output = `${output}获取到 ${length} 个成绩\n`
      let maxpp = 0
      let minpp = 0
      let perfectcount = 0
      const summary_mods = [] // {mod, count, maxpp}
      for (let i = 0; i < length; i++) {
        // 记录rank
        const rank = scoreObjects[i].rank
        const rankIndex = rank_list.indexOf(rank)
        ...
        const mods = utils.getScoreModsStringArray(scoreObjects[i].mods)
        mods.map((mod) => {
          const summary_mods_Index = summary_mods.findIndex(value => value.mod === mod)
          if (summary_mods_Index < 0) {
            ...
            if (pp > summary_mods[summary_mods_Index].maxpp)
              summary_mods[summary_mods_Index].maxpp = pp
          }
        })
        if (scoreObjects[i].full_combo)
          perfectcount += 1
      }
      for (let i = 0; i < rank_list.length; i++) {
        if (rank_count[i] > 0)
          output = `${output + rank_count[i]} 个 ${rank_list[i]}\n`
      }
      ...
      if (sbCount > 0)
        output = `${output}其中有 ${sbCount} 个S评级的成绩不是full combo`
      return output
    }
    catch (ex) {
      return ex
