...
class getBestScoresData {
  ...
  async outputToday() {
    try {
      ...
      const scoreObjects = await this.getBestScoresObject()
  ...
  async output() {
    try {
      if (this.apiObject.limit) {
        // 指定bp
        const scoreObjects = await this.getBestScoresObject()
        const scoreObject = scoreObjects.pop()
        let output = ''
        // output = output + scoreObject.toCompleteString(parseInt(this.apiObject.m));
        output = output + await scoreObject.toCompleteString(Number.parseInt(this.apiObject.m), true)
        return output
      }
      else {
        // bp列表
  ...
  async outputRankNumber() {
    try {
      // bp列表
      let output = 'bp成绩统计：\n'
      const scoreObjects = await this.getAllBestScoresObject()
      const rank_list = ['XH', 'X', 'SH', 'S', 'A', 'B', 'C', 'D']
      ...
      if (sbCount > 0)
        output = `${output}其中有 ${sbCount} 个S评级的成绩不是满combo`
      return output
    }
    catch (ex) {
      return ex
  ...
  async outputBpNumber() {
    try {
      ...
      const scoreObjects = await this.getAllBestScoresObject()
      const length = scoreObjects.length
      const beatmapId = this.apiObject.b.toString()
      for (let i = 0; i < length; i++) {
        if (beatmapId === scoreObjects[i].beatmap_id) { // 都是字符串格式
          output = output + await scoreObjects[i].toCompleteString(Number.parseInt(this.apiObject.m), true)
