...
class OsusearchApi {
  ...
  static findtheMostSuitable(result, params) {
    ...
    if (params.diff_name) {
      const diff_result = result.filter((beatmap) => {
  ...
  static async search(_data, returnType = 'id') {
    ...
    try {
      ...
      if (result.beatmaps.length > 1)
        resultBeatmap = this.findtheMostSuitable(result.beatmaps, params)
      else resultBeatmap = result.beatmaps[0]
      if (returnType === 'id')
        return resultBeatmap.beatmap_id
      else if (returnType === 'set')
        return resultBeatmap.beatmapset_id
