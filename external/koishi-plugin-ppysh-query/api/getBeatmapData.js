...
class getBeatmapData {
  ...
  async getBeatmapObject(mods = 0) {
    ...
    if (beatmaps.code === 404)
      throw `找不到谱面 ${utils.apiObjectToString(this.apiObject)}`
    if (beatmaps.code === 'error')
      throw `获取谱面出错 ${utils.apiObjectToString(this.apiObject)}`
  ...
  async outputPool(mods = 0) {
    try {
      const beatmapObject = await this.getBeatmapObject(mods)
