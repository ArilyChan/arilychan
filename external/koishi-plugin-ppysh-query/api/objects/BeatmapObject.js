...
class BeatmapObject {
  constructor(beatmap, mods = 0) {
    ...
    this.hit_length = Number.parseInt(beatmap.hit_length)
  ...
  async calculateWithMods(mode) {
    ...
    const mapCalculater = await new MapCalculater(this.beatmapId, ppv2Options).initFullPP()
    const map = mapCalculater.map
    if (map.artist_unicode !== '')
      this.artist_unicode = map.artist
    if (map.title_unicode !== '')
      this.title_unicode = map.title
    if (mode === 0) {
      const ar = map.ar
      const od = map.od
      const hp = map.hp
