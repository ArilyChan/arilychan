...
class getUserData {
  ...
  async getUserObject(nedb) {
    ...
    if (user.length <= 0)
      throw `获取玩家出错 ${utils.apiObjectToString(this.apiObject)}`
    const userObject = new UserObject(user[0])
    // 存储userObject
    const mode = (this.apiObject.m) ? Number.parseInt(this.apiObject.m) : 0
  ...
  async output(nedb) {
    try {
      const user = await this.getUserObject(nedb)
      if (typeof user === 'string')
        return user // 报错消息
      const mode = (this.apiObject.m) ? Number.parseInt(this.apiObject.m) : 0
      // 从数据库里寻找beforeUserInfo
      const beforeUserInfo = await new UserInfo(this.host, this.apiKey).getBeforeUserObject(nedb, user.userId, mode)
