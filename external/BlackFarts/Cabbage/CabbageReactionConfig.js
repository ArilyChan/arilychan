...
const usage = {
  ...
  'dad': '!(dad, dad_of) <osu用户名或id>',
  // findTeam: `【叹号】(team.find, findteam) <队伍名>`,
  // findTeamsByRank: `【叹号】(team.findbyrank, findteamsbyrank, findteambyrank) <分段>`,
  'say': '!say <...something>',
  '吃啥': '!吃啥 [菜单] [特殊需求]',
  '吃点刺激的': '!吃点刺激的 [菜单] [特殊需求]',
  '加个菜': '!加个菜 <菜单> <菜名> (换行)[中人的图片]',
  '倒': '!倒 <菜单> <菜名>',
  'menu.marknsfw': '!menu.marknsfw <菜单>',
  'recipe.marknsfw': '!recipe.marknsfw <菜单> <菜名>',
  'menu.marksfw': '!menu.marksfw <菜单>',
...
module.exports = {
  ...
  say: async ({ command, meta, app }) => {
    ...
    if (config.isManager(meta.userId))
      meta.send(message).catch(e => console.error.bind(console))
    else if (config.isEnabled('say', meta.userId))
      meta.send(`${meta.sender.nickname}: ${message}`).catch(e => console.error.bind(console))
  },
}
