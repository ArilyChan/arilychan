class utils {
  ...
  static getUserTimePlayed(play_time) { // play_time为秒数
    const s = Number.parseInt(play_time)
    if (s <= 0)
      return '从来没玩过'
    ...
    if (day > 0)
      output = `${output + day}天`
    if (hour > 0)
      output = `${output + hour}小时`
    if (minute > 0)
      output = `${output + minute}分`
  ...
  static format_number(n) {
    const b = Number.parseInt(n).toString()
    const len = b.length
    if (len <= 3)
      return b
    const r = len % 3
  ...
  static getScoreMods(enabledMods) {
    const raw_mods = Number.parseInt(enabledMods)
    const Mods = {
      None: 0,
      NoFail: 1,
      Easy: 1 << 1,
  ...
  static getScoreModsString(enabledMods) {
    ...
    // V2放后面
    if (hasV2)
      modsString = `${modsString}, ScoreV2`
    // relax放最后面
    if (hasRelax)
      modsString = `${modsString}, Relax`
    return modsString
  }
  ...
  static getModeString(mode) {
    if (mode !== 0 && mode !== '0' && !mode)
      return '当你看到这条信息说明代码有漏洞惹'
    const modeString = mode.toString()
    if (modeString === '0')
      return 'Standard'
    else if (modeString === '1')
      return 'Taiko'
  ...
  static gethitLengthString(hitlength) {
    const hl = Number.parseInt(hitlength)
  ...
  static apiObjectToString(apiObject) {
    let output = '（'
    if (apiObject.b)
      output = `${output} 谱面Id：${apiObject.b}`
    if (apiObject.u)
      output = `${output} 玩家：${apiObject.u}`
    if (apiObject.type)
      output = `${output} 玩家名类型：${apiObject.type}`
    if (apiObject.m)
      output = `${output} 模式：${this.getModeString(apiObject.m)}`
    if (apiObject.limit)
      output = `${output} 索引：${apiObject.limit}`
    output = `${output} ）`
    return output
  }
}
