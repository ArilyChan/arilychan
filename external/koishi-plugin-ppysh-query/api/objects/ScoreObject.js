...
class ScoreObject {
  constructor(score, beatmapId) {
    ...
    this.maxcombo = Number.parseInt(score.maxcombo)
    this.count50 = Number.parseInt(score.count50)
    this.count100 = Number.parseInt(score.count100)
    this.count300 = Number.parseInt(score.count300)
    this.countmiss = Number.parseInt(score.countmiss)
    this.countkatu = Number.parseInt(score.countkatu)
    this.countgeki = Number.parseInt(score.countgeki)
    this.perfect = Number.parseInt(score.perfect) // 0,1

    this.mods = Number.parseInt(score.enabled_mods)
    this.rank = score.rank
    this.pp = Number.parseFloat(score.pp) || 0 // recent没有提供pp
  ...
  async toCompleteString(mode, exScore = false) {
    ...
    if (mode === 0) { // std
      if (this.count300 > 0)
        counts.push(` ${this.count300}x 300 `)
      if (this.count100 > 0)
        counts.push(` ${this.count100}x 100 `)
      if (this.count50 > 0)
        counts.push(` ${this.count50}x 50 `)
      if (this.countmiss > 0)
        counts.push(` ${this.countmiss}x miss `)
    }
      ...
      if (this.count300 > 0)
        counts.push(` ${this.count300}x 水果 `)
      if (this.count100 > 0)
        counts.push(` ${this.count100}x 中果 `)
      if (this.count50 > 0)
        counts.push(` ${this.count50}x 果粒 `)
      if (this.countmiss > 0)
        counts.push(` ${this.countmiss}x miss水果 `)
      if (this.countkatu > 0)
        counts.push(` ${this.countkatu}x miss果粒 `)
    }
    else if (mode === 3) { // mania
      if (this.countgeki > 0)
        counts.push(` ${this.countgeki}x 彩300 `)
      if (this.count300 > 0)
        counts.push(` ${this.count300}x 300 `)
      if (this.countkatu > 0)
        counts.push(` ${this.countkatu}x 200 `)
      if (this.count100 > 0)
        counts.push(` ${this.count100}x 100 `)
      if (this.count50 > 0)
        counts.push(` ${this.count50}x 50 `)
  ...
  calACC(mode) {
    if (mode === 1) {
      ...
      this.acc = total === 0 ? 0 : (((this.count300 + this.count100 * 0.5) * 300) / (total * 300) * 100)
    }
    if (mode === 2) {
  ...
  async extendScore(mode) {
    ...
    const mapCalculater = await new MapCalculater(this.beatmap_id, ppv2Options).init(mode)
    const map = mapCalculater.map
    if (map.artist_unicode == '')
      map.artist_unicode = map.artist
    if (map.title_unicode == '')
