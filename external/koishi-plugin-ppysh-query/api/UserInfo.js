...
class UserInfo {
  ...
  async saveUserObject(nedb, newUserObject, mode) {
    ...
    // 查找是否已有记录
    const res = await nedb.findOne({ $and: [{ userId }, { mode }] })
    if (!res) {
      // 初始化记录
      await nedb.insert({ userId, userName: newUserObject.username, mode, beforeUserObject: newUserObject.toJson(), afterUserObject: newUserObject.toJson() })
    }
    else {
      // 更新userObject
      const afterUserObject = new UserObject().init(res.afterUserObject, mode)
      if (afterUserObject.getDateString() === newUserObject.getDateString())
        await this.updateToday(nedb, userId, newUserObject.toJson(), mode)
  ...
  async getBeforeUserObject(nedb, userId, mode = 0) {
    const res = await nedb.findOne({ $and: [{ userId }, { mode }] })
    if (res) {
      if (res.beforeUserObject)
        return new UserObject().init(res.beforeUserObject, mode)
      return undefined
    }
  ...
  async bindUser(nedb, qqId, apiObject) {
    try {
      let output = ''
      // 检查是否绑定过
      const res = await nedb.find({ qqId })
      if (res.length > 0) {
      ...
      const userObject = await new getUserData(this.host, this.apiKey, apiObject).getUserObject(nedb)

      // 这时数据库已记录该玩家，添加qqId字段即可
      // 检查该玩家是否已被绑定其他玩家
      const userId = userObject.userId
      const res2 = await nedb.find({ userId })
      if (res2.length > 0) {
        if (res2[0].qqId) {
          // 正在绑定其他QQ
        ...
        await nedb.update({ $and: [{ userId }, { mode: 0 }] }, { $set: { qqId } })
        await nedb.update({ $and: [{ userId }, { mode: 1 }] }, { $set: { qqId } })
        await nedb.update({ $and: [{ userId }, { mode: 2 }] }, { $set: { qqId } })
        await nedb.update({ $and: [{ userId }, { mode: 3 }] }, { $set: { qqId } })
        ...
        if (apiObject.m || apiObject.m === 0) {
          ...
          const mode = Number.parseInt(apiObject.m)
  ...
  async getUserOsuInfo(qqId, nedb) {
    const res = await nedb.find({ qqId })
    if (res.length > 0) {
      const defaultMode = res[0].defaultMode || 0
      return { qqId, osuId: res[0].userId, osuName: res[0].userName, defaultMode }
    }
