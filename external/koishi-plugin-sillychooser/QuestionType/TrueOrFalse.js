...
function getAVWords(result, start = 0) {
  ...
  if (index < 0)
    return { wordsIndex: -1 }
    // 向前包括助词和人称代词
  if ((index > 0) && (result[index - 1].w === '没' || result[index - 1].w === '不'))
    index = index - 1
  if ((index > 0) && (result[index - 1].p === 0x10000))
    index = index - 1
...
function AVWords2Choices(av) {
  ...
  if (av.AIndex < av.VIndex) {
    // 形容词很简单，在前面加不就行了
    if (av.words[av.AIndex] === '不')
      return [av.words.substr(0, av.AIndex) + av.words.substr(av.AIndex + 1), av.words]
    return [av.words, `${av.words.substr(0, av.AIndex)}不${av.words.substr(av.AIndex)}`]
  }

  // 第一个是动词
  if (av.VIndex < av.AIndex) {
    if (av.words[av.VIndex] === '不')
      return [av.words.substr(0, av.VIndex) + av.words.substr(av.VIndex + 1), av.words]
    if (av.words[av.VIndex] === '没')
      return [`${av.words.substr(0, av.VIndex) + av.words.substr(av.VIndex + 1)}了`, av.words] // “了”放最后
    if (av.words.substr(av.VIndex, av.VLength).endsWith('不了'))
      return [`${av.words.substr(0, av.VIndex) + av.words.substr(av.VIndex, av.VLength - 2)}得了${av.words.substr(av.VIndex + av.VLength)}`, av.words]
    if (av.words.substr(av.VIndex, av.VLength).endsWith('得了'))
      return [av.words, `${av.words.substr(0, av.VIndex) + av.words.substr(av.VIndex, av.VLength - 2)}不了${av.words.substr(av.VIndex + av.VLength)}`]
    if (av.words[av.VIndex + av.VLength - 1] === '了')
      return [av.words, `${av.words.substr(0, av.VIndex)}没${av.words.substr(av.VIndex, av.VLength - 1)}${av.words.substr(av.VIndex + av.VLength)}`]
...
function TrueOrFalse(askObject) {
  ...
  if (yIndex < 1)
    return reply.no() // 单独一个“吗”字也不行

  // 判断语气词y前一个词
  if (result[yIndex - 1].w === '什么')
    return reply.no() // 不是选择性疑问句
  ...
  else if (isResultContainWord(result, '是'))
    return reply.setChoices(['是', '不是']) // 直接从原句中找“是”更好，因为分析有可能不会单独拆下“是”，下面几个同理
  else if (isResultContainWord(result, '会'))
    return reply.setChoices(['会', '不会'])
  else if (isResultContainWord(result, '能'))
    return reply.setChoices(['能', '不能'])
