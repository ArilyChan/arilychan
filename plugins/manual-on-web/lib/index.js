...
function RouteHOC(Comp) {
  return (props = {}) => {
    ...
    const [pending, start] = solidJs.useTransition();
...
const _tmpl$ = ["<head><title>\uD83D\uDD25 Solid SSR \uD83D\uDD25</title><meta charset=\"UTF-8\"><meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><link rel=\"stylesheet\" href=\"/ssr/styles.css\">", "</head>"],
      _tmpl$2 = ["<html", " lang=\"en\">", "<body><div id=\"app\"><ul class=\"inline\"><li class=\"", "\">", "</li><li class=\"", "\">", "</li><li class=\"", "\">", "</li></ul><div class=\"", "\">", "</div></div></body><script type=\"module\" src=\"/ssr/js/client.js\" async></script></html>"],
      _tmpl$3 = ["<h2", ">Error: <!--#-->", "<!--/--></h2>"],
...
const Settings = solidJs.lazy(() => Promise.resolve().then(function () { return require('./Settings-6a3acc78.js'); }));
const App = RouteHOC(() => {
  const [, pending, {
    matches
  }] = solidJs.useContext(RouterContext);
...
const _boundary = path__default["default"].join(process.env.NODE_PATH || process.cwd(), 'node_modules');
...
module.exports.apply = function (ctx) {
  ...
  ctx.router.get('/manual/plugin/:plugin', async _ctx => {
    ...
    const boundary = fs__default["default"].realpathSync(path__default["default"].resolve(_boundary, plugin)); // _ctx.response.body = 'plugin: ' + plugin + '\n'
    // _ctx.response.body += 'boundary:' + boundary + '\n'

    let pluginPath = fs__default["default"].realpathSync(require.resolve(plugin)); // _ctx.response.body += 'main file:' + pluginPath + '\n'

    let relative = path__default["default"].relative(boundary, pluginPath);
    let inScope = relative && !relative.startsWith('..') && !path__default["default"].isAbsolute(relative);
    let possibleReadme = path__default["default"].resolve(pluginPath, './readme.md');
    let gotReadme = fs__default["default"].existsSync(possibleReadme); // _ctx.response.body += 'possible readme:' + possibleReadme + '\n'
    // _ctx.response.body += 'gotReadme: ' + gotReadme + '\n'

    if (gotReadme) {
      _ctx.response.body += marked.parse(fs__default["default"].readFileSync(possibleReadme, {
        encoding: 'utf8'
      }));
    } else {
      while (!gotReadme && inScope) {
        pluginPath = path__default["default"].resolve(pluginPath, '..');
        relative = path__default["default"].relative(boundary, pluginPath);
        inScope = relative && !relative.startsWith('..') && !path__default["default"].isAbsolute(relative);
        possibleReadme = path__default["default"].resolve(pluginPath, './readme.md'); // _ctx.response.body += 'possible readme:' + possibleReadme + '\n'

        gotReadme = fs__default["default"].existsSync(possibleReadme); // _ctx.response.body += 'gotReadme: ' + gotReadme + '\n'
      }
  ...
  ctx.router.get('/ssr/', async _ctx => {
    ...
    try {
      result = await web.renderToStringAsync(() => web.createComponent(App, {
        get url() {
          return req.url;
        }
  ...
  ctx.router.get('/ssr/:file(.*)', async _ctx => {
    ...
    const p = path__default["default"].resolve(__dirname, '../dist/public', file);
    const content = await fs__default["default"].promises.readFile(p, {
      encoding: 'utf8'
    }).catch(err => {
    ...
    const m = mime.lookup(p); // console.log(content, m)
