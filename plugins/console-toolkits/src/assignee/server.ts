...
export default function (ctx: Context) {
  ...
  async function nameOfChannelOrGuild (channelOrGuildId: string, supplementaryQueries?: {platform: Bot['platform'], assignee: Bot['selfId']}) {
    ...
    // }
    return Promise.all(bots.map(async (bot) => await bot.getChannel(channelOrGuildId).then(c => c.channelName).catch(noop) || await bot.getGuild(channelOrGuildId).then(g => g.guildName).catch(noop)))
