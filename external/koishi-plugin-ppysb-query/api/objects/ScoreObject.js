...
class ScoreObject {
  ...
  constructor(score, beatmap, user) {
    if (score.beatmap)
      this.beatmap = new BeatmapObject(score.beatmap, true)
    else this.beatmap = beatmap
    if (score.user)
      this.user = new SimpleUserObject(score.user)
    else this.user = user
  ...
  toString() {
    ...
    const ppString = (this.pp === 0 || this.completed === 0) ? '' : `${Number.parseFloat(this.pp).toFixed(2)}pp`
  ...
  async extendScore() {
    if (this.mode === 0) {
      ...
      const mapCalculater = await new MapCalculater(this.beatmap.beatmapId, ppv2Options).init(this.mode)
