...
const osusearch_1 = require("../package/osusearch");
...
class Arg {
    ...
    async getBeatmapInfo() {
        if (!this.message)
            throw new ErrorMessage('请输入正确格式：artist - title(mapper)[diff_name] 或直接给出beatmapSetId，参数只有纯数字title请在前后加上双引号');
        ...
        // 直接给出setId
        if (searchData.beatmapSet)
            beatmapSetId = searchData.beatmapSet;
        // 用sayobot搜索谱面setId
        else if (searchData.sayoTitle) {
            const result = await sayobot_1.sayobotApi.searchList(searchData.sayoTitle);
            if (result.code) {
        ...
        // 用osusearch搜索谱面setId
            ...
            const result = await osusearch_1.OsusearchApi.search(searchData);
        ...
        // 用sayobot获取谱面信息
        const beatmapInfo = await sayobot_1.sayobotApi.search(beatmapSetId, diffName);
