...
class getUserData {
  ...
  async getUserObject(nedb) {
    ...
    if (rippleUser.code === 404)
      throw `找不到玩家 ${utils.apiObjectToString(this.apiObject)}`
    if (rippleUser.code === 400)
      throw '必须指定玩家名或Id（或先setid绑定私服账户）'
    if (rippleUser.code === 'error')
      throw `获取玩家出错 ${utils.apiObjectToString(this.apiObject)}`
    const userObject = new UserObject(rippleUser)
    // 存储userObject
  ...
  async getSimpleUserObject() {
    ...
    if (rippleUser.code === 'error')
      throw `获取玩家出错 ${utils.apiObjectToString(this.apiObject)}`
    const userObject = new SimpleUserObject(rippleUser)
    return userObject
  }

  async output(nedb) {
    try {
      const rippleUser = await this.getUserObject(nedb)
      if (typeof rippleUser === 'string')
        return rippleUser // 报错消息
      const mode = (this.apiObject.m) ? this.apiObject.m : undefined
      // 从数据库里寻找beforeUserInfo
      const beforeUserInfo = await new UserInfo(this.host).getBeforeUserObject(nedb, rippleUser.userId, this.isRX)
