...
class Arg {
  ...
  constructor(args) {
    ...
    if (args.userStringWithBeatmap)
      this.users = this.getUsers(args.userStringWithBeatmap)
    if (args.userStringWithoutBeatmap)
      this.users = this.getUsers(args.userStringWithoutBeatmap)
    if (args.modsString)
      this.mods = this.getEnabledModsValue(args.modsString)
    if (args.modeString || args.modeString === 0)
      this.mode = this.getMode(args.modeString)
    if (args.onlyModeString || args.onlyModeString === 0)
      this.mode = this.getMode(args.onlyModeString)
    if (args.limitString)
      this.limit = Number.parseInt(args.limitString)
  }
  ...
  getBeatmapInfo(beatmapString) {
    ...
    if (this.checkInt(beatmapString)) { this.beatmapId = Number.parseInt(beatmapString) }
    else {
      let s = beatmapString
  ...
  getUsers(userString) {
    ...
    if (typeof userString === 'number') {
      // 是从数据库获取的
      if (userString > 0)
        users.push(userString.toString())
      // 数据库找不到就是空
      return users
    }
  ...
  getMode(modeString) {
    ...
    else if (s.includes('std'))
      return 0
    else if (s.includes('standard'))
      return 0
    else if (s.includes('click'))
      return 0
    else if (s.includes('泡泡'))
      return 0
    else if (s.includes('taiko'))
      return 1
    else if (s.includes('鼓'))
      return 1
    else if (s.includes('catch'))
      return 2
    else if (s.includes('ctb'))
      return 2
    else if (s.includes('接'))
      return 2
    else if (s.includes('mania'))
      return 3
    else if (s.includes('key'))
      return 3
    else if (s.includes('骂娘'))
      return 3
    else if (s.includes('琴'))
      return 3
    else if (s === 's')
      return 0
    else if (s === 't')
  ...
  getOsuApiObject() {
    ...
    if (users.length > 0) {
      options = users.map((singleuser) => {
        ...
        if (this.beatmapId > 0)
          option.b = this.beatmapId
        if (this.mode || this.mode === 0)
          option.m = this.mode
        if (this.mods || this.mods === 0)
          option.mods = this.mods
        if (this.limit)
  ...
  async getBeatmapId() {
    if (this.beatmapId > 0)
      return this
    if (Object.keys(this.beatmapSearchInfo).length <= 0)
      return this
    const id = await OsusearchApi.search(this.beatmapSearchInfo)
    if (typeof id === 'number') {
      this.beatmapId = id
    }
    else {
      if (id.code === 404)
        throw `找不到谱面：${JSON.stringify(this.beatmapSearchInfo)}`
  ...
  async getBeatmapSetId() {
    ...
    if (Object.keys(this.beatmapSearchInfo).length <= 0)
      return this
    const id = await OsusearchApi.search(this.beatmapSearchInfo, 'set')
    if (typeof id === 'number') {
      this.beatmapId = id
    }
    else {
