...
class EventsJsonUtils {
    ...
    writeJson(file, eventsJson) {
        ...
        fs_1.default.writeFileSync(file, str, 'utf8');
    }
    ...
    async runAdd(meta, eventPath, users, name, good, bad, app) {
        ...
        if (users.admin && users.admin.indexOf(meta.userId) >= 0)
            isAdmin = true;
        if (users.blackList && users.blackList.indexOf(meta.userId) >= 0)
            atBlackList = true;
        if (users.whiteList && users.whiteList.indexOf(meta.userId) >= 0)
            atWhiteList = true;
        if (isAdmin || atWhiteList)
            return this.addEvent(meta, eventPath, name, good, bad);
        else if (atBlackList)
            return '抱歉，我讨厌你';
        else {
    ...
    async runDel(meta, eventPath, users, name) {
        ...
        if (users.whiteList && users.whiteList.indexOf(meta.userId) >= 0)
            atWhiteList = true;
        if (isAdmin || atWhiteList)
            return this.delEvent(meta, eventPath, name);
        else
            return '抱歉，您没有权限，无法删除活动';
    }
...
exports.default = EventsJsonUtils;
