...
class UserObject {
  ...
  init(userObjectJson) {
    const thisUserObject = JSON.parse(userObjectJson)
  ...
  tocompareString(oldUserObject, mode = this.favourite_mode, isRx) {
    ...
    if (isRx)
      output = `${output}模式：Relax\n`
    else output = `${output}模式：Classic\n`
    output = `${output}id：${this.userId}\n`
    output = output + this.modeStats[Number.parseInt(mode)].compareTo(oldUserObject.modeStats[Number.parseInt(mode)])
    output = `${output}\n`
