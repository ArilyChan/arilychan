...
async function compileMenu({ storage }) {
  ...
  const mixin = all.reduce((acc, recipe) => {
    ...
    if (!acc[recipe.menu.name])
      acc[recipe.menu.name] = []
...
const recipeToString = function (order) {
  ...
  if (order.description)
    message.push(order.description, '\n')
  if (order.uploader.name)
    message.push('推荐人: ', order.uploader.name, '\n')
  // if (order.uploader.qq) message.push('qq: ', order.uploader.qq, '\n')
  if (order.cqImage)
    message.push(order.cqImage, '\n')
...
function echoRecipe(meta, req, menu = compiledMenu) {
  ...
  if (meta.contentType !== 'private')
    message.push(new CQ.Reply().id(meta.messageId))
  if (!req) {
...
async function recipe({ command, meta, storage }) {
  const [, req] = command
  if (!menuCompiled)
    await compileMenu({ storage })
  echoRecipe(meta, req, compiledMenu.filter(item => !item.nsfw && !item.menu.nsfw))
}

async function nsfwRecipe({ command, meta, storage }) {
  const [, req] = command
  if (!menuCompiled)
    await compileMenu({ storage })
...
async function refreshMenu({ command, meta, storage }) {
  compiledMenu.length = 0
  return compileMenu({ storage }).then(() => meta.send('ok')).catch(e => console.error.bind(console))
}

async function addRecipe({ command, meta, storage }, consumptionMethod = '吃') {
  try {
    const [, name, ...others] = command
    const originalMenuNames = Object.keys(storage.originalMenu)
    if (originalMenuNames.includes(name))
      return meta.send('不能动这个!!').catch(e => console.error.bind(console))
    let { recipeSection, cqImage } = others.join(' ').split('[CQ:image,').reduce((acc, startsWithCQImage, index) => {
      ...
      if (rest)
        acc.recipeSection.push(rest.replace('\r', '\n'))
      return acc
    ...
    if (!recipe)
      return meta.send('<menu> <recipe> [图片]').catch(e => console.error.bind(console))
    ...
    if (newrecipe)
      return meta.send('menu exists!').catch(e => console.error.bind(console))
    const recipeContent = {
      name: recipe,
      description,
    ...
    newrecipe.save((err, saved) => {
      if (err)
        meta.send(err).catch(e => console.error.bind(console))
      else meta.send('ok').catch(e => console.error.bind(console))
      if (!storage.menu[name])
        storage.menu[name] = []
...
async function editRecipe({ command, meta, storage }) {
  ...
  if (!methodOfConsumption)
    return meta.send('?').catch(e => console.error.bind(console))

  const { Recipe, Menu } = storage.menuModels

  menu = await Menu.findOne({ name: menu }).exec()
  if (!menu)
    return meta.send('there\'s no such menu').catch(e => console.error.bind(console))

  recipe = await Recipe.findOne({ name: recipe, menu: menu._id }).exec()
  if (!recipe)
    return meta.send('there\'s no such recipe in the menu').catch(e => console.error.bind(console))
  ...
  if (inCompiledMenu)
    inCompiledMenu.methodOfConsumption = methodOfConsumption
  meta.send('好了好了我知道了')
}

async function removeRecipe({ command, meta, storage }) {
  let [, name, ...recipe] = command
  recipe = recipe.join(' ')
  if (!recipe)
    return meta.send('<menu> <recipe>').catch(e => console.error.bind(console))
  const { Recipe: RecipeModel, Menu: MenuModel } = storage.menuModels
  let menu = await MenuModel.findOne({ name }).exec()
  if (!menu) {
    menu = new MenuModel({ name })
  ...
  if (!newrecipe)
    return meta.send('menu non-exists!').catch(e => console.error.bind(console))
  ...
  if (ptrRecipe)
    compiledMenu.splice(ptrRecipe, 1)
}
...
const markNSFWMenuBot = async function ({ command, meta, storage }, isNSFW) {
  const [, name] = command
  if (!name)
    return meta.send('<menu>').catch(e => console.error.bind(console))
  const result = await markNSFWMenu(storage, name, isNSFW)
  if (!result)
    meta.send('menu non-exists').catch(e => console.error.bind(console))
